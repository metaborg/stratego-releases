...
imports strategy sugar lib display

signature
  constructors
    Clos : List(Prod([String,Term])) * Term -> Term

strategies

  main = iowrap(eval-application)

  eval-application =
    where(<create-display> "senv");
    App(id, \ t -> Clos([], t) \ ); 
    eval;
    try(\Clos([], t) -> t\ );
...
// Strategy definition and call
  ...
  EvalCall :
    ...
    where <display-lookup> ("senv", f) => SDef(f, xs, s);
          <zip(\ (x, a) -> 
                 <display-push> ("senv", x, SDef(x,[],a)) \ )> (xs, as)

  EvalLet :
    App(Let(sdefs, s), t) -> App(Seq(s, UndefineDefs(sdefs)), t)
    where <map(\sdef@SDef(f,_,_) -> <display-push>("senv", f, sdef)\ )> sdefs
  ...
  EvalUndefine :
    ...
    where <map(\ x -> <display-pop>("senv", x)\ )> xs

  EvalUndefine :
    App(UndefineDefs(defs), t) -> t
    where <map(\ SDef(f,_,_) -> <display-pop>("senv", f)\ )> defs
