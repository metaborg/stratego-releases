...
strategies
  ...
  Repeat(s1, s2) = 
	{x: Rec(?x, LChoice(repeat(AssociateL); 
  ...
  Repeat(s1, s2) =
	{x, s1', str: 
         Rec(?x, LChoice(repeat(AssociateL); 
...
strategies
  ...
  // this is too simplistic; the strategy expression could contain 
  // a call to a strategy that does do a transformation. This requires
  // effects analysis.
