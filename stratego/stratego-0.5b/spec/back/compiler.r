...
\begin{code}
  C : Instr(Let(sdefs, s), env, rcs) ->
      Block([Instr(s, env, rcs),
             Goto(y),
	     Block(<map(\ sdef -> Instr(sdef, env, rcs)\ )> sdefs),
	     Label(y)])
      where new => y
  ...
  C :Instr(sdef@SDef(x, [_|_], s), env, rcs) ->
     Block([])
     where <debug(!"local strategy with arguments: ")> sdef;
           <fatal-error>["not yet supported"]
...
\begin{code}
  ...
  C : BuildTerm(Op(f, ts), env, rcs) -> ATmakeApplN(lab, args)
      where <length> ts => len; <leq> (len, 6);
            <map(\ t -> BuildTerm(t, env, rcs) \ )> ts => args;
            <get-afun> AFun(f, len) => lab
