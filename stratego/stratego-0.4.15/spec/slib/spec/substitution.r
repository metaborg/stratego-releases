...
\end{code}
	...
	The strategy 
	\verb|<substitute(...)> ([(x1,t1),...,(xn,tn)],t)| 
	replaces the \verb|xi| by the corresponding \verb|ti|.
...
\end{code}
	...
	Here is another version of substitute that also renames
	the bound variables in the terms it substitutes for variables.
	This version can be used to keep 

\begin{code}
strategies

  substitute-rn(isvar, bndvars, boundin, paste) = 
    ?(sbs, t); !(t, []);
    rec x(env-alltd(RnVar(isvar)
                    <+ (SubsVar(isvar, !sbs), id); x
                    <+ RnBinding(bndvars, paste);
                       DistBinding(boundin, x)))
\end{code}
