...
$Id: svm.h,v 1.10 2000/09/20 08:08:26 visser Exp $
...
#define GOBOOL(s) if((s) == ATfalse) goto fail;

#define GO(s) GOBOOL(s)

#define GOFAIL(s, k) if((s) == ATfalse) goto *(k);
...
// ATbool _TestFunFC(char *f, int n, ATerm);
ATbool _TestFunFC(AFun, ATerm);

ATbool _TestIntFC(int n, ATerm);
ATbool _TestRealFC(double, ATerm);
ATbool _TestStringFC(char *, ATerm);

#define MatchVar(i)          GO(_MatchVar(i))
#define MatchVard(i, o)      GO(_MatchVard(i, o))
#define MatchInt(i)          GO(_MatchInt(i))
#define MatchIntFC(i, fc)    GOFAIL(_MatchIntFC(i), fc)
#define MatchReal(i)         GO(_MatchReal(i))
#define MatchRealFC(i, fc)   GOFAIL(_MatchRealFC(i), fc)
#define MatchString(s)       GO(_MatchString(s))
#define MatchStringFC(s, fc) GOFAIL(_MatchStringFC(s), fc)
#define MatchFun(f, n)       GO(_MatchFun(f, n))
#define MatchFunFC(f, n, fc) GOFAIL(_MatchFunFC(f, n), fc)

// #define TestFunFC(f, n, fc, t)    GO(_TestFunFC(f, n, fc, t))
#define TestFunFC(f, fc, t)    GOFAIL(_TestFunFC(f, t), fc)

#define TestIntFC(n, fc, t)    GOFAIL(_TestIntFC(n, t), fc)
#define TestRealFC(n, fc, t)   GOFAIL(_TestStringFC(n, t), fc)
#define TestStringFC(n, fc, t) GOFAIL(_TestStringFC(n, t), fc)

/* Building */

#define NotNULLd(i, o) \
  {if(Egetd(i, o) == NULL) goto fail;}
...
#define IsAppl if(!t_is_appl(Ttop)) {goto fail;};
...
#define NextSon(x)     GOFAIL(_NextSon(), x)
// #define SomeNextSon(x) GO(_SomeNextSon(x, &&fail))
...
#define OneNextSon   GOBOOL(_NextSon())
...
void MatchTravInit(void);
ATbool _MatchNextSon(void);

#define OneMatchNextSon GOBOOL(_MatchNextSon())
...
ATerm doit(ATerm t) \
{ \
  Tinitst; Einit; CounterInit; Tset(t); 
...
#define DOIT \
int _stratego_init_afuns(void); \
\
ATerm doit(ATerm t) \
{ \
ATfprintf(stderr, "doit(%t)\n", t); \
     TprintStackTop(5,"a"); \
  Tinitst; Einit; CounterInit; Tset(t); \
     TprintStackTop(5,"a"); \
  ...
  _stratego_main(); \
     TprintStackTop(5,"a"); \
\
  ATfprintf(stderr, "** strategy succeeded\n"); \
  exit(0); \
  fail : \
    if(csp > 0) {Crestore; Cjump;} \
    if(csp == 0) {\
