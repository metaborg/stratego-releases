...
strategies
 ...
 make-option-table =
   ...
   map(\ x@opt#(_) -> <display-push>("options", opt, x)\ )
 ...
 ifdebug(s) =
   try(where(<display-lookup>("options", "Debug")); s)

 iftrace(s) =
   try(where(<display-lookup>("options", "Trace")); s)
 ...
 copyright =
    where((<display-lookup> ("options", "DeclVersion") <+ !DeclVersion("")) 
          => DeclVersion(version));
    say(!"--------------------------------------------------------------------");
  ...
  create-runtime-environment =
    where(<create-display> "options"); // command-line options
    where(<create-display> "senv"); // strategy variables
    where(<create-display> "tenv")  // term variables

  destroy-runtime-environment =
    where(<destroy-display> "options");
    where(<destroy-display> "senv");
    where(<destroy-display> "tenv")
  ...
  define-strategies =
    map(\ def@SDef(f, xs, _) -> <display-push>("senv", (f,<length>xs), def)\ )
  ...
  eval-dump =
    ...
    say(<conc-strings> ("image written to file: ", file))
