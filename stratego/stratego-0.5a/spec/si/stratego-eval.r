...
rules
  ...
  EvalScope(eval) :
    App(Scope(xs, s), t) -> t'
    where <map(\ x -> <display-push>("tenv", (Var(x),0), NULL())\ )> xs;
	  <eval> App(s, t) => t';
          <map(\ x -> <display-pop>("tenv", (Var(x),0))\ )> xs
...
rules
  ...
  BindVar :
    (x, t) -> (x, t)
    where //debug(!"BindVar: "); 
          (<display-lookup> ("tenv", x) => NULL();
           <display-replace> ("tenv", x, t))
          <+ <display-push> ("tenv", x, t)
...
// Term traversal
  ...
  EvalAll(eval) :
    App(All(s), f#(ts)) -> f#(ts')
    where <map(\ t -> <eval> App(s, t)\; not(Fail) )> ts => ts'
